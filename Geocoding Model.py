...
# Define city center coordinates (example coordinates for London)
city_center_coords = (51.5074, -0.1278)
...
df['distance_to_center'] = df.apply(lambda row: geodesic((row['latitude'], row['longitude']), city_center_coords).km, axis=1)

# Select relevant features for regression
features = ['availability_365', 'reviews_per_month', 'rating', 'number_of_reviews', 'number_of_stays', '5_stars', 'review_frequency', 'price_to_availability', 'distance_to_center']
X = df[features]
y = df['price']
